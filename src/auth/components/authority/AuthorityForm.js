...
class AuthorityForm extends Component {
	constructor(props) {
		super(props)
		this.searchParams = qs.parse(props.location.search.substring(1))
		this.state = {
	...
	//搜索的方法
	search(value) {
		api.get('/rbac/getResourcesByType?type_id=' + 1 + '&app_id=' + this.searchParams.app_id + '&value=' + value).then((response) => {
			this.setState({
...
export default Form.create()(withRouter(AuthorityForms))
