...
class Authority extends Component {
	...
	addRoleAuthority(record) {
		if (record.type_id) {
			api.get('/rbac/getResourcesByType?type_id=' + record.type_id+'&app_id='+this.state.app_id).then((response) => {
				this.setState({
					type: 'add',
					title: '添加权限',
					okText: '提交',
			...
			api.get('/rbac/getResourceTypeList?app_id='+this.state.app_id).then((response) => {
