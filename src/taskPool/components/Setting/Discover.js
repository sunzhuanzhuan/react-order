        ...
        <Checkbox.Group defaultValue={arr}>
          {saveArrTime.map((item, index) => {
            return <p key={index}>
              ...
              <Button type="link" onClick={() => this.handleOpenModal(false, item)} btnDisable={this.props.btnDisable}>删除</Button>
            </p>
          })}
        </Checkbox.Group>
        <p><Button type="primary" onClick={() => this.handleOpenModal(true)} btnDisable={this.props.btnDisable}>新增保留时长</Button></p>
        <Modal
          title="新增保留时长"
          visible={visible}
          onOk={this.handleOk}
