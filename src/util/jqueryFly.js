...
let init = function ($) {
	$.fly = function (element, options) {
		...
		self.setOptions = function (options) {
			...
			$element
				.css({ marginTop: "0px", marginLeft: "0px", position: "fixed" })
				.appendTo("body")
			// 运动过程中有改变大小
			...
			var distance = Math.sqrt(
				...
				// 元素移动次数
				steps = Math.ceil(
					Math.min(Math.max(Math.log(distance) / 0.05 - 75, 30), 100) / settings.speed
				),
				ratio =
			...
			if (end.width != null && end.height != null) {
				var i = steps / 2,
					width =
						...
						Math.cos(count < i ? 0 : (((count - i) / (steps - i)) * Math.PI) / 2),
					height =
						end.height -
